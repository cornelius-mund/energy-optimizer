...
""",
    ...
    monkeypatch.setenv("ENERGY_OPTIMIZER_CONFIG", str(configuration))
    invalid_requests = [
        {**household_load_request(), "load_kw": []},
        {**household_load_request(), "load_kw": [-0.1]},
        {**household_load_request(), "interval_minutes": 30},
        {**household_load_request(), "start_time": "2026-01-01T00:00:00"},
        {**household_load_request(), "schema_version": "2"},
        {**household_load_request(), "unexpected": True},
    ]
...
def test_household_load_contract_rejects_more_than_one_week(
    tmp_path: Path, monkeypatch: MonkeyPatch
) -> None:
    configuration = tmp_path / "config.yaml"
    configuration.write_text(
